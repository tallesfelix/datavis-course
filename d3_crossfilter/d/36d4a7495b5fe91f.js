...
d3.csv("https://gist.githubusercontent.com/emanueles/d8df8d875edda71aa2e2365fae2ce225/raw/1e949d3da02ed6caa21fe3a7a12a4e5a611a4bab/stocks.csv").then(function(data){
  ...
  data.forEach(function(d,i){
    ...
    if(iGoogle == null && iFacebook == null){
      iGoogle = +d.google;
      iFacebook =  +d.facebook;
      d.pGoogle = 0;
      d.pFacebook = 0; 
    } else {
      d.pGoogle   =   (( (+d.google) - iGoogle) / iGoogle) * 100
      d.pFacebook =   (( (+d.facebook) - iFacebook) / iFacebook) * 100
    }
   })
  return data
  return data
...
dateDim.group().reduceSum(d => d.facebook)
  ...
  main.variable(observer("percentGoogle")).define("percentGoogle", ["dateDim"], function(dateDim){return(
dateDim.group().reduceSum(d => d.pGoogle)
)});
  main.variable(observer("percentFacebook")).define("percentFacebook", ["dateDim"], function(dateDim){return(
dateDim.group().reduceSum(d => d.pFacebook)
)});
  ...
  main.variable(observer("buildcomposite")).define("buildcomposite", ["md","container","dc","d3","dateDim","googleByDayGroup","fbByDayGroup"], function(md,container,dc,d3,dateDim,googleByDayGroup,fbByDayGroup)
{
  ...
  main.variable(observer("acoes")).define("acoes", ["md","container","dc","d3","dateDim","percentGoogle","percentFacebook"], function(md,container,dc,d3,dateDim,percentGoogle,percentFacebook)
{
  ...
  compositeChart.width(800)
              ...
              .compose([
                  dc.lineChart(compositeChart)
                    .group(percentGoogle, 'Google')
                    .ordinalColors(['steelblue']),
                  dc.lineChart(compositeChart)
                    .group(percentFacebook, 'Facebook')
                    .ordinalColors(['darkorange'])])
  dc.renderAll()
  return view      
}
...
d3.json("https://raw.githubusercontent.com/emanueles/datavis-course/master/assets/files/observable/movies.json").then(function(data){
  ...
  main.variable(observer("movies")).define("movies", ["crossfilter","datasetMovies"], function(crossfilter,datasetMovies){return(
crossfilter(datasetMovies)
)});
  main.variable(observer("yearDim")).define("yearDim", ["movies"], function(movies){return(
movies.dimension(d => d.Year)
)});
  main.variable(observer("genreDim")).define("genreDim", ["movies"], function(movies){return(
movies.dimension(d => d.Genre)
)});
  main.variable(observer("wwgrossDim")).define("wwgrossDim", ["movies"], function(movies){return(
movies.dimension(d => d.Worldwide_Gross_M)
)});
  main.variable(observer("grossByYear")).define("grossByYear", ["yearDim"], function(yearDim){return(
yearDim.group().reduceSum(d => d.Worldwide_Gross_M)
)});
  main.variable(observer("grossByGenre")).define("grossByGenre", ["genreDim"], function(genreDim){return(
genreDim.group().reduceSum(d => d.Worldwide_Gross_M)
)});
  main.variable(observer("by_year")).define("by_year", ["md","container","dc","yearDim","d3","grossByYear"], function(md,container,dc,yearDim,d3,grossByYear)
{
  ...
  main.variable(observer("by_genre")).define("by_genre", ["md","container","dc","yearDim","d3","grossByGenre"], function(md,container,dc,yearDim,d3,grossByGenre)
{
  let view = md`${container('chart5', 'Total Arrecadado nas Bilheteria por Gênero de 2007 a 2011 (em Milhões de Dólares)')}`
