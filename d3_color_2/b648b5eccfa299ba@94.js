  ...
  main.variable(observer("geojson")).define("geojson", ["L","info","map","bairros","style"], function(L,info,map,bairros,style)
{
  ...
  main.variable(observer("legend")).define("legend", ["L","oranges","colorScale","d3","map"], function(L,oranges,colorScale,d3,map)
{
  let legendControl = L.control({position: 'bottomright'});
	...
	legendControl.onAdd = function (map) {
		...
		for (let i = 0; i < n; i++) {
			...
			labels.push(
				'<i style="background:' + oranges[i] + '"></i> ' +
...
<main role="main" class="container">
 ...
 `
  ...
  main.variable(observer("oranges")).define("oranges", ["d3"], function(d3){return(
d3.schemeOranges[9]
)});
  main.variable(observer("colorScale")).define("colorScale", ["d3","oranges"], function(d3,oranges){return(
d3.scaleQuantize()
    .domain([0, 425.13])
    .range(oranges)
)});
