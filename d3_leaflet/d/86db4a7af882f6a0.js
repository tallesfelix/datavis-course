...
d3.csv("https://gist.githubusercontent.com/emanueles/65a308ffa630689c11a031252998ef8d/raw/a004c770786229d54264406118ae21ba7e4c51a8/earthquakes.csv").then(function(data) {
  ...
  data.forEach(function(d,i){
       ...
       d.profundidade  = depthFormat(+d.depth)
  })
...
facts.dimension(d => d.magnitude)
  ...
  main.variable(observer("profundidadeDimension")).define("profundidadeDimension", ["facts"], function(facts){return(
facts.dimension(d => d.profundidade)
)});
...
magnitudeDimension.group()
  ...
  main.variable(observer("profundidadeGroup")).define("profundidadeGroup", ["profundidadeDimension"], function(profundidadeDimension){return(
profundidadeDimension.group()
)});
  main.variable(observer("hourGroup")).define("hourGroup", ["hourDimension"], function(hourDimension){return(
hourDimension.group()
)});
  main.variable(observer("buildvis")).define("buildvis", ["md","container","dc","dateDimension","d3","magnitudeDimension","magnitudeGroup","profundidadeDimension","profundidadeGroup","dataset","hourDimension","hourGroup"], function(md,container,dc,dateDimension,d3,magnitudeDimension,magnitudeGroup,profundidadeDimension,profundidadeGroup,dataset,hourDimension,hourGroup)
{
  ...
  let profundidadeBar = dc.barChart(view.querySelector('#depth-chart'))
  let terremotosLine  = dc.lineChart(view.querySelector('#time-chart'))
   ...
   dataTable
    ...
    .size(10)
    .columns(['dtg', 'magnitude', 'profundidade','latitude','longitude'])
    .sortBy(d => d.dtg)
    .order(d3.ascending)
  ...
  profundidadeBar
    ...
    .gap(1)
    .elasticY(true)
    .dimension(profundidadeDimension)
  ...
  dataTable.render()
  magnitudeBar.render()
  profundidadeBar.render()
  terremotosLine.render()
